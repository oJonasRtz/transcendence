...
const authControllers = {
  ...
  tryLoginTheUser: async function tryLoginTheUser(req, reply) {
    ...
    try {
      ...
      const res = await axios.post("https://users-service:3003/getPublicId", {
        user_id: user_id,
      });
  ...
  createNewToken: async function createNewToken(req, reply) {
    try {
      if (
        ...
        return reply
          .code(400)
          .send("Bad request, you need to inform email, user_id and username");
      const { username, email, user_id } = req.body;
      const response = await axios.post(
        "https://users-service:3003/getPublicId",
        { user_id: user_id }
      );
  ...
  checkRegister: async function tryRegisterTheUser(req, reply) {
    ...
    const error = [];
    try {
      const { user_id, username, nickname, email, password, confirmPassword } =
        req.body;
      ...
      await axios.post("https://sqlite-db:3002/deleteUserAccount", {
  ...
  get2FAEnable: async function get2FAEnable(req, reply) {
    try {
      ...
      const result = await axios.post(
        "https://sqlite-db:3002/get2FAEnable",
        req.body
      );
      return reply
        .code(200)
        .send({ twoFactorEnable: result?.data?.twoFactorEnable ?? null });
    } catch (err) {
  ...
  get2FASecret: async function get2FASecret(req, reply) {
    try {
      if (!req.body || !req.body.email)
        return reply.code(400).send("You need to inform an e-mail here");
      const result = await axios.post(
        "https://sqlite-db:3002/get2FASecret",
        req.body
      );
      console.log("o result da auth secret:", result);
      return reply
        .code(200)
        .send({ twoFactorSecret: result?.data?.twoFactorSecret ?? null });
    } catch (err) {
  ...
  get2FAValidate: async function get2FAValidate(req, reply) {
    try {
      if (!req.body || !req.body.email)
        return reply.code(400).send("You need to inform an e-mail here");
      const result = await axios.post(
        "https://sqlite-db:3002/get2FAValidate",
        req.body
      );
      return reply
        .code(200)
        .send({ twoFactorValidate: result?.data?.twoFactorValidate ?? null });
    } catch (err) {
  ...
  set2FAValidate: async function set2FAValidate(req, reply) {
    try {
      if (!req.body || !req.body.email || req.body.signal === undefined)
        return reply
          .code(400)
          .send("You need to inform an email and signal here");
  ...
  getAuthData: async function getAuthData(req, reply) {
    try {
      ...
      const data = await axios.post(
        "https://sqlite-db:3002/getAuthData",
        req.body
      );
  ...
  setAuthUsername: async function setAuthUsername(req, reply) {
    try {
      if (!req.body || !req.body.user_id || !req.body.username)
        return reply
          .code(400)
          .send("You need to inform your user_id and username");
      await axios.post("https://sqlite-db:3002/setAuthUsername", req.body);
  ...
  setAuthNickname: async function setAuthNickname(req, reply) {
    try {
      if (!req.body || !req.body.user_id || !req.body.nickname)
        return reply
          .code(400)
          .send("You need to inform your user_id and nickname");
      await axios.post("https://sqlite-db:3002/setAuthNickname", req.body);
  ...
  setAuthPassword: async function setAuthPassword(req, reply) {
    try {
      if (!req.body || !req.body.user_id || !req.body.password)
        return reply
          .code(400)
          .send("You need to inform your user_id and the new password");
      await axios.post("https://sqlite-db:3002/setAuthPassword", req.body);
  ...
  deleteUserAccount: async function deleteUserAccount(req, reply) {
    try {
      ...
      await axios.post("https://sqlite-db:3002/deleteUserAccount", req.body);
  ...
  set2FAOnOff: async function set2FAOnOff(req, reply) {
    try {
      ...
      const response = await axios.post(
        "https://sqlite-db:3002/set2FAOnOff",
        req.body
      );
      if (response?.data.message === "2FA_ENABLED")
        return reply.code(200).send({ message: "2FA_ENABLED" });
      else return reply.code(200).send({ message: "2FA_DISABLED" });
    } catch (err) {
  ...
  helloDb: async function testCommunicationAuthWithDatabase(req, reply) {
    try {
      ...
      return reply.send(
        `Auth-service confirms communication with sqlite-db ${response.data}`
      );
      console.log("Success communicating with database");
    } catch (err) {
      console.error(
        "The auth-service cannot communicate correctly with sqlite-db"
      );
