...
export async function validatorHook(req, reply) {
  ...
  const check = [
    ...
    {
      condition:
        req.body.captchaInput !== undefined &&
        req.body.captchaInput !== null &&
    ...
    },
    {
      condition:
        req.body.nickname &&
        (!usernameRegex.test(req.body.nickname) ||
          req.body.nickname.length < 3 ||
          req.body.nickname.length > 20),
...
export async function authHook(req, reply) {
  ...
  const isApiRequest = req.url?.startsWith("/api/") || wantsJson;
  ...
  try {
    ...
    if (twoFactorEnable?.data.twoFactorEnable) {
      ...
      if (!twoFactorValidate?.data.twoFactorValidate) {
        const qrCodeDataURL = await axios.post(
