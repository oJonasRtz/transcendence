...
export default async function registerServer(io) {
  ...
  io.use((socket, next) => {
    ...
    if (!cookie) return next(new Error("No cookies sent"));
  ...
  async function getTournamentAdvise() {
    try {
      const response = await axios.get(
        "https://match-service:3010/tournaments"
      );
      if (response?.data.tournament) {
        ...
        await axios.post("https://chat-service:3005/storeMessage", {
          name: `SYSTEM`,
          isSystem: true,
          avatar: "/app/public/images/default_avatar.png",
          isLink: false,
          msg: match,
        });
      }
  ...
  async function reloadEverything(owner) {
    try {
      ...
      const responseMessages = await axios.post(
        "https://chat-service:3005/getAllMessages",
        { username: owner }
      );
      let data = Array.isArray(responseMessages?.data)
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("disconnect", async () => {
      ...
      try {
        ...
        await axios.post("https://chat-service:3005/storeMessage", {
          ...
          isLink: false,
          msg: `system: ${data.name} left the chat`,
        });
    ...
    socket.on("sendPrivateInvite", async (target_id) => {
      ...
      try {
        ...
        await axios.post("https://chat-service:3005/storePrivateMessage", {
          user_id: socket.user_id,
          avatar:
            userAvatar?.data.avatar ?? "/app/public/images/default_avatar.png",
          isLink: true,
          msg: invitation?.data.link,
          public_id: target_id,
        });

        const response = await axios.post(
          "https://chat-service:3005/getAllPrivateMessages",
          { user_id: socket.user_id, public_id: target_id }
        );
        ...
        /*try {
                                	res = await axios.post("https://users-service:3003/getDataByPublicId", { public_id: target_id });
                                	target_name = res?.data.username;
				} catch (err) { return ; }*/

        //let response = null;
        /*try {
					response = await axios.post("https://chat-service:3005/getAllPrivateMessages", { user_id: socket.user_id, public_id: target_id });
				} catch (err) {}
        ...
        if (!invitation?.data.link) {
          ...
          notifications.push({
            isSystem: true,
            isLink: false,
            content: `system: You need to wait time to send another link`,
            username: "SYSTEM",
    ...
    socket.on("sendInvite", async () => {
      ...
      try {
        ...
        if (!response?.data.link) await reloadEverything(socket.username);
        notifications.length = 0;
        notifications.push({
          isSystem: true,
          isLink: false,
        ...
        socket.emit("updateNotifications", notifications);
        console.error(
          `Error sending the pong invite match, user: ${socket.username}:`,
          err
        );
      }
    ...
    socket.on("sendMessage", async (msg) => {
      ...
      try {
        ...
        let input = msg?.trim();
        if (!input || input.length > 200) throw new Error("LENGTH_TOO_HIGH");

        const senderName = socket.username;

        if (!senderName) return;

        blockUserTargets = blacklist
          .filter((target) => target.owner_username === senderName)
          .map((user) => user.target_username); // obtain all the names of users' blocked
        ...
        if (err.message === "LENGTH_TOO_HIGH") {
          ...
          notifications.push({
            ...
            isLink: false,
            isLimit: true,
            msg: "system: You cannot type a message above 200 characters",
            username: "SYSTEM",
            avatar: "/public/images/system.png",
          });
          socket.emit("updateNotifications", notifications);
        ...
        console.error(
          `Error trying to send the message of user ${socket.username}:`,
          err
        );
      }
    ...
    socket.on("sendPrivateMessage", async (msg, public_id) => {
      ...
      notifications.length = 0;
      try {
        if (msg && msg.length > 200) throw new Error("TOO_HIGH_LENGTH");
        ...
        await axios.post("https://chat-service:3005/storePrivateMessage", {
          user_id: socket.user_id,
          avatar: ress?.data.avatar ?? "/app/public/images/default_avatar.png",
          isLink: false,
          msg: msg,
          public_id: public_id,
        });
        const response = await axios.post(
          "https://chat-service:3005/getAllPrivateMessages",
          { user_id: socket.user_id, public_id: public_id }
        );
