...
export default function WaitingLobby({ user }: WaitingLobbyProps) {
  ...
  // Pega o token opcional da URL
  const token = searchParams.get('token');

  // Conecta ao match e entra na party
  useEffect(() => {
    const joinLobby = async () => {
      try {
        ...
        // Chama joinParty via API
        await fetch('/api/match-service', {
          method: 'POST',
        ...
        // Pega os players da party
        const res = await fetch(`/api/match-service?user_id=${user.public_id}`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        setPlayers(
          (data.party?.clients || []).map((p: any) => ({
            public_id: p.id,
            username: p.name,
            nickname: p.name,
            avatar: `/uploads/avatar_${p.id}.png`,
            isOnline: true,
            isHost: p.isLeader,
          }))
        );
    ...
    // Ao desmontar, sai da party
    return () => {
      fetch('/api/match-service', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'leaveParty',
  ...
  // Atualiza players periodicamente do match.party
  useEffect(() => {
    const interval = setInterval(() => {
      setPlayers(
        match.party.map((p: any) => ({
          public_id: p.id,
          username: p.name,
          nickname: p.name,
          avatar: `/uploads/avatar_${p.id}.png`,
  ...
  return (
    <div className="space-y-6">
      {!match.isConnected && <MatchProvider />}
        ...
        <button
          onClick={() => {
            fetch('/api/match-service', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ action: 'leaveParty', id: user.public_id }),
              credentials: 'include',
            }).catch(console.error);
