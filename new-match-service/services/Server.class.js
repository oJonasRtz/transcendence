...
export class Server {
	...
	#handlers = {
		'CONNECT': this.#addClient.bind(this),
	}
	...
	#handleMessages({data, client, ws}) {
		const c = this.#clients.get(ws) || ws;
		const isClient = c instanceof Client;
		try {
			const {type} = data;
...
// 		{ method: 'POST', url: '/invite', handler: this.#invite.bind(this) }
// 	];
// 	#room = new Map(); // <room_id, {game_type: 'RANKED' | 'TOURNMENT', owner: Client, state: 'LOBBY | 'QUEUE' | 'IN_GAME', guest?: Client}>
// 	#clients = new Map(); // <ws, Client>
// 	#handlers = {
...
// 		if (![owner, game_type, state].every(Boolean)
// 				|| !['RANKED', 'TOURNAMENT'].includes(game_type)
// 				|| !['LOBBY', 'QUEUE', 'IN_GAME'].includes(state)
// 				|| !(owner instanceof Client))
// 			return 1;

// 		const room_id = crypto.randomUUID();
// 		this.#room.set(room_id, { owner, game_type, state });
